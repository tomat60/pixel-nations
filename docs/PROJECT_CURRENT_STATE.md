...
Current product phase: accepted Aurelian Minimal Economy Foundation v1 research; implementation preflight only.
Current milestone: issue #719 must prove the exact seven-file Aurelian Provision Allocation v1 scope, state/persistence migration, workflow triggers and evidence topology.
Active execution issue: #719
Next allowed action: complete and record the mandatory implementation preflight on issue #719. No product implementation branch is authorized. Even `IMPLEMENTATION_PREFLIGHT_PASS` requires a separate merged authority before implementation.

## Accepted Aurelian Minimal Economy Foundation v1 research

PR #718 received terminal `AURELIAN_MINIMAL_ECONOMY_FOUNDATION_V1_RESEARCH_PASS` at exact head `fe9e46fe6e7abdba587d88516590a46641a024e1` and merged as `9ffecc7b60817c38e112b882c9074b00049b19e1`. It changed exactly the two authorized documentation files; CI, RC1, Visual QA, P4-P8, P10-P11 and Vercel passed. Issue #715 is completed and issue #719 is the only active preflight issue. Product and runtime remain unchanged.

Issue #715 compared Aurelian Provision, Construction Timber and Trade Cargo. It selects Aurelian Provision Allocation v1: exactly one Provision is acquired by the accepted first-caravan dispatch and later allocated either to the existing North Ridge Outpost or held as a committed Greenvale Reserve. One persisted enum records the destination; quantity is derived and can never exceed one. The two outcomes are geographically distinct and must be visible in Village, Map and World through one canonical provision presentation, not HUD copy alone.

The proposed later implementation ceiling is seven exact files. Implementation remains blocked until issue #719 records `IMPLEMENTATION_PREFLIGHT_PASS` and a separate authority merges. Full findings and gates are in `docs/AURELIAN_MINIMAL_ECONOMY_FOUNDATION_V1_RESEARCH.md`.
