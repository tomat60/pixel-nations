...
Status: ACTIVE
Updated: 2026-08-10
Current state revision: Aurelian Topology Lock v0.3
...
The accepted web product preserves the complete founder progression through Founder Record and one previous read-only history. P4–P11 are accepted and merged. P11 closed the bounded P10 continuity defect; it does not authorize P12 or another retention roadmap.
...
- Vercel deployment status for the product baseline: `SUCCESS`.
- Independent public-origin checks for `/`, `/play`, and `/world`: `PRODUCTION UNVERIFIED`. The current control environment rejected the public origin as unsafe to open; this is missing release evidence, not evidence of an outage. The production steward must retry it on later runs.
...
After every merge, verify accepted head → merge SHA → new `main` → checks → deployment → real public routes. Localhost RC1 and pre-merge preview are not post-release proof. If the public origin cannot be tested, record `PRODUCTION UNVERIFIED` and the exact missing evidence.
...
- read-only inventory of reusable Godot state/persistence code and verified KayKit provenance;
- control-plane documentation and release verification;
- retrying independent public-origin checks.
...
- `PASS`: authorize one bounded Phase 1 Godot shared-geography proof under #415; or
- `CORRECTION REQUIRED`: perform the one named correction pass and review again; or
