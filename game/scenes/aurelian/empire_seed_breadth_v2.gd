...
const BREADTH_MANIFEST_PATH := "res://scenes/aurelian/empire_seed_breadth_v2_manifest.json"
...
func _ready() -> void:
	...
	if sector_spec.is_empty() or breadth_manifest.is_empty():
		push_error("EMPIRE_SEED_BREADTH_V2_MANIFEST_LOAD_FAILED")
		get_tree().quit(81)
		return
	if String(breadth_manifest.get("contract", "")) != "EMPIRE_SEED_BREADTH_V2":
		push_error("EMPIRE_SEED_BREADTH_V2_CONTRACT_INVALID")
		get_tree().quit(82)
		return
	_index_territories()
	evidence_dir = OS.get_environment("AURELIAN_EVIDENCE_DIR")
	if OS.get_environment("AURELIAN_CAPTURE_EMPIRE_SEED_BREADTH_V2") == "1":
...
func _territory_position(territory: Dictionary, y := MARKER_Y) -> Vector3:
	var locus := _locus_by_id(String(territory.get("id", "")))
	if locus.is_empty():
		return Vector3.ZERO
	var center: Array = locus.get("center", [])
	if center.size() != 2:
		return Vector3.ZERO
	return _sector_to_godot(Vector2(float(center[0]), float(center[1])), y)

func _add_owned_domain(parent: Node3D, territory: Dictionary) -> void:
	if String(territory.get("status", "")) != "ours":
		return
	var radius := float(territory.get("domain_radius", 0.0))
	if radius <= 0.0:
		return
	var disc := MeshInstance3D.new()
	disc.name = "OwnedDomain_%s" % String(territory.get("id", ""))
	...
	disc.mesh = mesh
	disc.position = _territory_position(territory, OVERLAY_Y)
	var color := _palette_color("ours")
	color.a = 0.18
	disc.material_override = _make_unshaded_material(color)
	parent.add_child(disc)

func _add_marker(parent: Node3D, territory: Dictionary) -> void:
	if not bool(territory.get("show_marker", true)):
		return
	var locus_id := String(territory.get("id", ""))
	var locus := _locus_by_id(locus_id)
	if locus.is_empty():
		push_warning("EMPIRE_SEED_BREADTH_V2_MISSING_LOCUS: %s" % locus_id)
		return
	...
	marker.position = _territory_position(territory)
	parent.add_child(marker)

	var ring := MeshInstance3D.new()
	ring.name = "OwnershipRing"
	var torus := TorusMesh.new()
	torus.inner_radius = radius * 0.68
	torus.outer_radius = radius
	torus.rings = 32
	torus.ring_segments = 12
	ring.mesh = torus
	ring.material_override = _make_unshaded_material(color, 0.22 if status == "ours" else 0.08)
	marker.add_child(ring)

	if status == "ours":
		var core := MeshInstance3D.new()
		core.name = "HomeCore" if locus_id == "aurelian_home" else "OwnedCore"
		var core_mesh := CylinderMesh.new()
		core_mesh.top_radius = radius * (0.38 if locus_id == "aurelian_home" else 0.28)
		core_mesh.bottom_radius = core_mesh.top_radius
		core_mesh.height = 0.28 if locus_id == "aurelian_home" else 0.18
		core_mesh.radial_segments = 12
		core.mesh = core_mesh
		core.position.y = 0.13
		core.material_override = _make_unshaded_material(color.lightened(0.16), 0.32)
		marker.add_child(core)
	elif status == "rival":
		var spike := MeshInstance3D.new()
		spike.name = "RivalPressure"
		var spike_mesh := CylinderMesh.new()
		spike_mesh.top_radius = 0.0
		spike_mesh.bottom_radius = radius * 0.24
		spike_mesh.height = 0.86
		spike_mesh.radial_segments = 8
		spike.mesh = spike_mesh
		spike.position.y = 0.48
		spike.material_override = _make_unshaded_material(color, 0.18)
	...
	if bool(territory.get("show_label", false)):
		...
		label.font_size = 72 if importance == "primary" else 62
		label.pixel_size = 0.0068 if importance == "primary" else 0.0060
		label.modulate = color.lightened(0.24)
		label.outline_modulate = Color("#17201de8")
		label.outline_size = 12
		label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
		label.no_depth_test = true
		label.position = Vector3(0.0, 0.92 if importance == "primary" else 0.76, 0.0)
...
func _add_route(parent: Node3D, route: Dictionary) -> void:
	...
	if from_center.size() != 2 or to_center.size() != 2:
		return
	var from_pos := _sector_to_godot(Vector2(float(from_center[0]), float(from_center[1])), OVERLAY_Y + 0.04)
	var to_pos := _sector_to_godot(Vector2(float(to_center[0]), float(to_center[1])), OVERLAY_Y + 0.04)
	var delta := to_pos - from_pos
	var midpoint := (from_pos + to_pos) * 0.5
	var priority := String(route.get("priority", "secondary"))
	var width := 0.20 if priority == "primary" else 0.10
	var route_mesh := MeshInstance3D.new()
	route_mesh.name = "Route_%s_%s" % [from_id, to_id]
	var beam := BoxMesh.new()
	beam.size = Vector3(width, 0.045, delta.length())
	...
	route_color.a = 0.90 if priority == "primary" else 0.62
	route_mesh.material_override = _make_unshaded_material(route_color, 0.07 if priority == "primary" else 0.02)
	parent.add_child(route_mesh)

func _build_strategic_layer(parent: Node3D) -> Node3D:
	var layer := Node3D.new()
	layer.name = "EmpireSeedStrategicLayerV2"
	parent.add_child(layer)
	for territory_variant in breadth_manifest.get("territories", []):
		_add_owned_domain(layer, territory_variant as Dictionary)
...
func _make_breadth_camera(parent: Node3D) -> Camera3D:
	...
	camera.name = "Camera_EmpireSeedBreadthV2"
...
func _capture_breadth() -> void:
	if evidence_dir.is_empty():
		push_error("EMPIRE_SEED_BREADTH_V2_MISSING_EVIDENCE_DIR")
		get_tree().quit(83)
		return
	DirAccess.make_dir_recursive_absolute(evidence_dir)
	var viewport := SubViewport.new()
	viewport.name = "EmpireSeedBreadthV2EvidenceViewport"
	...
	scene_root.name = "EmpireSeedBreadthV2EvidenceWorld"
	...
	if image == null or image.is_empty() or image.get_size() != BREADTH_STILL_SIZE:
		push_error("EMPIRE_SEED_BREADTH_V2_CAPTURE_FAILED")
		get_tree().quit(85)
		return
	var output_path := evidence_dir.path_join("empire-seed-breadth-v2-1440x900.png")
	if image.save_png(output_path) != OK:
		push_error("EMPIRE_SEED_BREADTH_V2_SAVE_FAILED")
		get_tree().quit(86)
		return
	print("EMPIRE_SEED_BREADTH_V2_STILL=%s" % output_path)
	print("EMPIRE_SEED_BREADTH_V2_TERRITORIES=%d" % territory_specs.size())
	print("EMPIRE_SEED_BREADTH_V2_STRATEGIC_MARKERS=4")
	get_tree().quit(0)
