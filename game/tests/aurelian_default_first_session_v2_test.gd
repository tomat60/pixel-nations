...
const CONTROLLER = preload("res://scenes/aurelian/playable_aurelian_first_session_v2.gd")
...
func _initialize() -> void:
	...
	_check(
		entry_scene.contains('path="res://scenes/aurelian/playable_aurelian_first_session_v2.gd"'),
		"entry_scene_uses_first_session_wrapper"
	)

	var manifest := _read_json(MANIFEST_PATH)
	var core_session: Dictionary = manifest.get("core_session", {})
	_check(String(core_session.get("contract", "")) == "AURELIAN_DEFAULT_FIRST_SESSION_V2", "core_session_contract")
	...
	var wrapper_source := _read_text("res://scenes/aurelian/playable_aurelian_first_session_v2.gd")
...
func _finish() -> void:
	if failures.is_empty():
		print("AURELIAN_DEFAULT_FIRST_SESSION_V2_TEST: PASS")
		quit(0)
		return
	for failure in failures:
		push_error("AURELIAN_DEFAULT_FIRST_SESSION_V2_TEST_FAILURE: %s" % failure)
	print("AURELIAN_DEFAULT_FIRST_SESSION_V2_TEST: FAIL (%d)" % failures.size())
	quit(1)
