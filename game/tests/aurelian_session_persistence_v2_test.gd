...
func _initialize() -> void:
	...
	_check(bool(missing.get("nation_founded", true)) == false, "missing_nation_fallback")
	...
	save_result = SESSION.save_session("world_first_trade_underway", "east_trade", TEST_PATH, true, true, true, true)
	_check(bool(save_result.get("ok", false)), "save_trade_underway_valid")
	restored = SESSION.load_session(TEST_PATH)
	...
	invalid = SESSION.save_session("world_first_trade_underway", "east_trade", TEST_PATH, true, true, true, false)
	...
	_check(source.contains('"village_trade_dispatched"'), "dispatch_village_state_allowed")
	...
	_check(source.contains('"world_first_nation_founded"'), "nation_world_state_allowed")
	_check(source.contains('"map_aurelian_homeland"'), "nation_map_state_allowed")
	_check(source.contains('"village_greenvale_capital"'), "nation_village_state_allowed")
	_check(source.contains('"city_chartered"'), "city_flag_persisted")
	_check(source.contains('"nation_founded"'), "nation_flag_persisted")
