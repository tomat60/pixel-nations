...
func _initialize() -> void:
	...
	_check(int(manifest.get("issue", 0)) == 510, "issue")
	var directions: Array = manifest.get("world_direction_choices", [])
	_check(directions == ["trade", "expand", "frontier"], "three_existing_directions")
	var controls: Dictionary = manifest.get("controls", {})
	_check(String(controls.get("inspect_previous_direction", "")) == "ui_up", "inspect_previous_control")
	_check(String(controls.get("inspect_next_direction", "")) == "ui_down", "inspect_next_control")
	_check(String(controls.get("commit_direction", "")) == "ui_accept", "commit_control")
	var commitment: Dictionary = manifest.get("commitment", {})
	_check(bool(commitment.get("explicit_player_action_required", false)), "explicit_only")
	_check(bool(commitment.get("exactly_one_direction", false)), "exactly_one_direction")
	_check(String(commitment.get("persistence_field", "")) == "national_direction", "persistence_field")
