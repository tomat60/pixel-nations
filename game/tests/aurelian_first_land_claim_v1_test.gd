...
func _initialize() -> void:
	...
	_check(forward_path.slice(0, 5) == ["world_neutral", "world_trade_selected", "map_east_route_selected", "map_east_route_claimed", "village_claimed"], "claim_forward_path_prefix")
	_check(forward_path.has("village_founded"), "founding_extends_claim_path")
	var backward_path: Array = playable.get("backward_path", [])
	_check(backward_path.has("map_east_route_claimed"), "claimed_backward_path_retained")
	_check(backward_path.has("world_trade_selected"), "world_backward_path_retained")
