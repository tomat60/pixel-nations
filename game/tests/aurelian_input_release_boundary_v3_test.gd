...
func _finish() -> void:
	if failures.is_empty():
		print("AURELIAN_INPUT_RELEASE_BOUNDARY_V3_TEST: PASS")
		quit(0)
		return
	for failure in failures:
		push_error("AURELIAN_INPUT_RELEASE_BOUNDARY_V3_TEST_FAILURE: %s" % failure)
	print("AURELIAN_INPUT_RELEASE_BOUNDARY_V3_TEST: FAIL (%d)" % failures.size())
	quit(1)
