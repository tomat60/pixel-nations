...
async function main() {
  ...
  try {
    ...
    await check("third sector unlocks nation in council", async () => { await runOrder(page, /^Form Council$/i); await openWorld(page); await page.locator('[data-qa="world-sector-tile"][data-sector-can-claim="true"]').first().click(); await claimSelected(page); const owned = await countControl(page, "owned"); if (owned < 3) throw new ExpansionQaError("third sector unlocks nation in council", `Expected 3 owned sectors, got ${owned}`); await page.locator('[data-qa="world-map-scene"][data-nation-ready="true"]').waitFor({ state: "visible", timeout: 7000 }); await clickButton(page, /^Council plan$/i, "open council"); await page.locator('[data-qa="council-nation-ready"]').waitFor({ state: "visible", timeout: 7000 }); await page.screenshot({ path: `${OUTPUT_DIR}/nation-ready.png`, fullPage: true }); });
