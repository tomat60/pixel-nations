...
const OUTPUT_DIR = "public/qa/p7-post-imperial-continuity-audit";
const REPORT_PATH = `${OUTPUT_DIR}/p7-post-imperial-continuity-audit-result.json`;
...
const STAGES = [
  {
    ...
    state: { empireCrisisRecoveryId: null, postCrisisCountermoveOrigin: null, postCrisisResponseId: null },
  },
  {
    id: "post-crisis-countermove",
    selector: '[data-qa="post-crisis-response"]',
    expectedObjective: /post-crisis counter-move/i,
    state: { empireCrisisRecoveryId: "stabilize-frontier", postCrisisCountermoveOrigin: "stabilize-frontier", postCrisisResponseId: null },
  },
  {
    id: "frontier-payoff",
    selector: '[data-qa="secure-post-crisis-frontier"]',
    expectedObjective: /Founder Record|another history|counter-move/i,
...
async function measure(page, viewportId, stage) {
  ...
  const readMetrics = () => target.evaluate((node) => {
    ...
    const belowFoldPx = Math.max(0, targetRect.bottom - viewportBottom);
    const aboveFoldPx = Math.max(0, viewportTop - targetRect.top);
...
async function runViewport(browser, viewport) {
  ...
  const stages = [];
  try {
    await page.goto(`${APP_URL}/play?qa-p7-post-imperial-continuity=${viewport.id}-${Date.now()}`, { waitUntil: "domcontentloaded", timeout: 10000 });
...
async function main() {
  ...
  try {
    ...
    assert(result.viewports.every((viewport) => viewport.stages.length === STAGES.length), "stage count", "Crisis, counter-move and frontier-payoff evidence are required for both viewports.");
    await writeResult("PASS");
    console.log(`P7 post-imperial continuity audit PASS. Evidence written to ${OUTPUT_DIR}`);
  } catch (error) {
    await writeResult("FAIL", error);
    console.error(`P7 post-imperial continuity audit FAIL at ${result.blockingStep}: ${result.error}`);
    process.exitCode = 1;
  } finally {
