...
const steps = [
  ...
  { id: "12-retention-state-after-reload", label: "Retention state after reload", note: "Reload preserves completed first era, city institutions, and world consequence markers.", run: async (page) => {
    ...
    await page.waitForLoadState("networkidle", { timeout: 5000 }).catch(() => {});
    await clickDock(page, "council");
    await page.locator('[data-qa="council-panel"][data-retention-count="3"][data-era-complete="true"][data-city-institutions="true"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="first-era-complete"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="city-institutions-seed"]').waitFor({ state: "visible", timeout: 5000 });
    const records = await page.locator('[data-qa="retention-record"]').count();
