...
const steps = [
  ...
  { id: "12-retention-state-after-reload", label: "Retention state after reload", note: "Reload preserves completed first era, city institutions, frontier objective, world objective marker, and world consequence markers.", run: async (page) => {
    await page.reload({ waitUntil: "domcontentloaded", timeout: 10000 });
    await page.waitForLoadState("networkidle", { timeout: 5000 }).catch(() => {});
    await clickDock(page, "council");
    ...
    await clickDock(page, "world");
    await page.locator('[data-qa="world-map-scene"][data-retention-count="3"][data-frontier-intent="northern-pass"][data-frontier-target-sector="A-02"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="world-frontier-objective-banner"][data-frontier-intent="northern-pass"][data-frontier-target-sector="A-02"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="world-sector-tile"][data-frontier-objective="true"][data-sector-id="A-02"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="world-frontier-objective-marker"]').waitFor({ state: "visible", timeout: 5000 });
    await page.locator('[data-qa="world-frontier-objective-signal"][data-frontier-intent="northern-pass"][data-frontier-target-sector="A-02"]').waitFor({ state: "visible", timeout: 5000 });
