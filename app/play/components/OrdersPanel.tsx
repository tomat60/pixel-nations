...
export function OrdersPanel({ state, dispatch }: { state: PlayState; dispatch: (action: PlayAction) => void }) {
  ...
  return (
    ...
    >
      ...
      {stewardshipUnlocked ? (
        <section data-qa="settlement-stewardship" className="mt-3 rounded-3xl border border-emerald-200/24 bg-emerald-950/28 p-3 shadow-inner md:p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <div data-qa="settlement-crew-total" className="rounded-xl border border-emerald-100/18 bg-black/28 px-2.5 py-1.5 text-right">
          ...
          <div className="mt-3 space-y-2">
            {settlementDistrictIds.map((districtId) => {
              ...
              return (
                <div
                  key={districtId}
                  data-qa={`settlement-district-${districtId}`}
                  className="flex min-w-0 items-center justify-between gap-2 rounded-2xl border border-emerald-100/14 bg-black/24 p-2.5"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-black text-amber-50">{meta.label}</p>
                    <p className="text-[10px] leading-relaxed text-amber-50/55">{meta.yield}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-1.5">
                    <button
                      ...
                      className="flex h-9 w-9 items-center justify-center rounded-xl border border-amber-100/16 bg-black/36 text-lg font-black text-amber-50 transition hover:border-amber-200/35 disabled:cursor-not-allowed disabled:opacity-30"
                    >
                      −
                    </button>
                    <span className="w-7 text-center text-lg font-black text-amber-50">{count}</span>
                    <button
                      type="button"
          ...
          <div className="mt-3">
            <p className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-100/55">Seasonal focus</p>
              ...
              {focusMeta.map((focus) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border p-2.5 text-left transition ${active ? "border-amber-200/55 bg-amber-200/14" : "border-amber-100/12 bg-black/22 hover:border-amber-200/30"}`}
                  >
                    <p className="text-xs font-black text-amber-50">{focus.label}</p>
                    <p className="mt-0.5 text-[10px] leading-relaxed text-amber-50/58">{focus.short}</p>
                  </button>
                );
              })}
            </div>
          ...
          <div data-qa="settlement-cycle-preview" className="mt-3 rounded-2xl border border-sky-100/18 bg-sky-950/24 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-[9px] font-black uppercase tracking-[0.18em] text-sky-100/58">Next-cycle forecast</p>
              <p className={`rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-[0.14em] ${!cycleReady ? "bg-amber-200/12 text-amber-100" : forecast.shortage ? "bg-rose-300/14 text-rose-100" : "bg-emerald-300/14 text-emerald-100"}`}>
                {!cycleReady ? "Assign all crews" : forecast.shortage ? "Shortage predicted" : "Stable season"}
              </p>
            </div>
            ...
            <p className="mt-2 text-[10px] leading-relaxed text-amber-50/58">
              {!cycleReady
                ? `Assign ${unassignedCrews} remaining ${unassignedCrews === 1 ? "crew" : "crews"} before resolving the season.`
                : forecast.shortage
                  ? `Available food will not cover ${forecast.upkeep} upkeep. Stability and prosperity will fall.`
                  : `Food after upkeep: ${forecast.netFood}. The settlement can safely advance.`}
            </p>
          </div>
          ...
          <button
            ...
            className="mt-3 w-full rounded-2xl border border-emerald-100/30 bg-emerald-300 px-4 py-3 text-sm font-black text-emerald-950 shadow-lg shadow-black/25 transition hover:bg-emerald-200 disabled:cursor-not-allowed disabled:opacity-45"
          >
            End season
          </button>

          {latestCycle ? (
            <div data-qa="settlement-cycle-record" className={`mt-3 rounded-2xl border p-3 ${latestCycle.shortage ? "border-rose-200/25 bg-rose-950/28" : "border-emerald-100/18 bg-black/24"}`}>
              <p className="text-[8px] font-black uppercase tracking-[0.17em] text-amber-100/52">Latest cycle · {latestCycle.cycle}</p>
              <p className="mt-1 text-xs font-bold leading-relaxed text-amber-50/78">{latestCycle.summary}</p>
              {latestCycle.orderId ? <p className="mt-1 text-[9px] text-amber-100/48">Project: {latestCycle.orderId}</p> : null}
            </div>
          ) : null}
        </section>
      ) : null}
