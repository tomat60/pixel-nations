...
type Camera = { x: number; y: number; zoom: number; dragging: boolean; startX: number; startY: number; originX: number; originY: number; raf: number | null };
...
export function MapStage({ state, dispatch }: MapStageProps) {
  ...
  return (
    <div className="absolute inset-0 touch-none">
      ...
      <svg viewBox="0 0 1000 900" preserveAspectRatio="xMidYMid meet" className="absolute inset-0 h-full w-full cursor-grab active:cursor-grabbing" role="img" aria-label="Aurelian Basin fullscreen map" onWheel={(event) => { event.preventDefault(); wheelZoom(event.deltaY); }} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); startDrag(event.clientX, event.clientY); }} onPointerMove={(event) => moveDrag(event.clientX, event.clientY)} onPointerUp={(event) => { event.currentTarget.releasePointerCapture(event.pointerId); endDrag(); }} onPointerCancel={endDrag} onPointerLeave={endDrag}>
        <defs>
