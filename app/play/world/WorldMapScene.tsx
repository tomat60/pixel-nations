...
export function WorldMapScene({ state, dispatch }: { state: PlayState; dispatch: (action: PlayAction) => void }) {
  ...
  return (
    ...
    >
      <div data-qa="expansion-hud" className="absolute left-3 right-3 top-[4.35rem] z-10 flex flex-col gap-1.5 md:left-4 md:right-4 md:top-[4.6rem] lg:flex-row lg:items-start lg:justify-between lg:gap-3">
        <div className="max-w-[720px] rounded-xl border border-sky-100/18 bg-black/42 px-3 py-1.5 shadow-lg backdrop-blur-md">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <p className="text-[8px] font-black uppercase tracking-[0.22em] text-sky-200/65">Expansion / Nation Loop v1</p>
            <h2 className="text-sm font-black text-amber-50 md:text-base">WorldMapScene · 10,000 lands</h2>
          </div>
          <p className="mt-0.5 hidden text-[11px] leading-snug text-amber-50/65 xs:block sm:block">Claim adjacent sectors with Influence. Each strategic sector expands into 100 generated lands.</p>
        ...
        <div className="flex flex-wrap gap-1 text-center"><Metric label="Owned" value={ownedSectorIds.length} /><Metric label="Influence" value={state.resources.influence} /><Metric label="Pressure" value={`${pressure}%`} /><Metric label="Claimable" value={claimableSectorIds.length} /><Metric label="Goal" value={nextGoal} /></div>
      </div>

      <div className="absolute bottom-[4.6rem] left-2 right-2 top-[8.6rem] flex flex-col gap-2 overflow-y-auto pb-6 md:bottom-[5.6rem] md:left-3 md:right-3 md:top-[9.2rem] lg:grid lg:grid-cols-[minmax(0,1fr)_320px] lg:overflow-hidden lg:pb-0">
        <div className="shrink-0 rounded-xl border border-sky-100/10 bg-black/12 p-2 md:p-2.5 lg:min-h-0">
          ...
          <div className="grid h-[520px] grid-cols-10 grid-rows-10 gap-1.5 md:h-[620px] md:gap-2 lg:h-[calc(100%-4.6rem)] lg:min-h-[440px]">{model.sectors.map((sector) => <SectorTile key={sector.id} sector={sector} control={getSectorControl(sector.id, ownedSectorIds, claimableSectorIds)} selected={sector.index === selected.index} nationFounded={nationFounded} institutionCount={institutionCount} objectiveTarget={sector.id === frontierTargetSectorId} objectiveComplete={frontierObjectiveComplete} obsidianPressure={obsidianPressure} onSelect={() => setSelectedIndex(sector.index)} canClaim={canClaimSector(state, sector.id).ok} />)}</div>
        </div>
