...
function SectorTile({ sector, control, selected, nationFounded, onSelect, canClaim }: { sector: WorldMapSector; control: SectorControl; selected: boolean; nationFounded: boolean; onSelect: () => void; canClaim: boolean }) {
  const foundedOwned = nationFounded && control === "owned";
  return <button type="button" data-qa="world-sector-tile" data-sector-id={sector.id} data-sector-x={sector.x} data-sector-y={sector.y} data-sector-kind={sector.kind} data-sector-control={control} data-sector-can-claim={canClaim ? "true" : "false"} data-sector-origin={sector.isOrigin ? "true" : "false"} data-sector-rival={sector.isRival ? "true" : "false"} data-sector-trade={sector.isTradeRich ? "true" : "false"} data-sector-danger={sector.isHighDanger ? "true" : "false"} data-nation-founded-owned={foundedOwned ? "true" : "false"} aria-label={`Inspect sector ${sector.id} ${sector.name}`} onClick={onSelect} className={`relative overflow-hidden rounded-xl border p-1 text-left transition hover:scale-[1.02] hover:border-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-200/80 ${kindClasses[sector.kind]} ${controlClasses[control]} ${selected ? "ring-2 ring-white" : ""} ${foundedOwned ? "outline outline-2 outline-emerald-200/80 shadow-[0_0_24px_rgba(52,211,153,.28)]" : ""}`}><span className="block text-[9px] font-black leading-none md:text-xs">{sector.id}</span><span className="mt-0.5 hidden truncate text-[8px] opacity-70 md:block">{sector.biome}</span><span className="absolute bottom-1 right-1 text-[8px] font-black opacity-70">{sector.trade}</span>{control === "claimable" ? <span className="absolute left-1 top-1 h-2 w-2 rounded-full bg-lime-200" /> : null}{sector.isHighDanger ? <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-red-300" /> : null}{foundedOwned ? <span className="absolute bottom-1 left-1 rounded-full bg-emerald-200 px-1 text-[8px] font-black text-stone-950">⚑</span> : null}</button>;
}
