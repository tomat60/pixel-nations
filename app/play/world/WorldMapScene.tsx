...
import { canClaimSector, expansionInfluenceCost, getClaimableSectorIds, getCourtCaseDecision, getEmpireDeclaration, getFrontierIntent, getFrontierObjectiveSecured, getImperialCourtCase, getNationDecision, getNationReady, getOwnedSectorIds, getRivalResponse, getRivalResponseDecision, type FrontierObjective, type PlayAction, type PlayState, type RetentionRecord } from "../lib/play-state";
    ...
    <section data-qa="world-map-scene" data-owned-count={ownedSectorIds.length} data-influence={state.resources.influence} data-nation-ready={nationReady ? "true" : "false"} data-nation-decision={nationDecision?.id ?? "none"} data-nation-founded={nationFounded ? "true" : "false"} data-frontier-intent={frontierObjective?.id ?? "none"} data-frontier-target-sector={frontierTargetSectorId ?? "none"} data-frontier-objective-complete={frontierObjectiveComplete ? "true" : "false"} data-empire-declaration={empireDeclaration?.id ?? "none"} data-empire-consequence={empireConsequence?.id ?? "none"} data-imperial-mandate={empireConsequence?.mandateId ?? "none"} data-court-case={courtCase?.id ?? "none"} data-court-case-decision={state.courtCaseDecisionId ?? "none"} data-rival-response={rivalResponse?.id ?? "none"} data-rival-response-decision={state.rivalResponseDecisionId ?? "none"} data-retention-count={state.retentionRecords.length} data-institution-count={institutionCount} data-world-lands={WORLD_LANDS} data-sector-count={SECTOR_COUNT} data-lands-per-sector={LANDS_PER_SECTOR} className="absolute inset-0 overflow-hidden bg-[radial-gradient(circle_at_18%_12%,rgba(56,189,248,.16),transparent_28%),radial-gradient(circle_at_82%_22%,rgba(251,191,36,.14),transparent_26%),linear-gradient(180deg,#07111b_0%,#030708_100%)]">
      ...
      <div data-qa="expansion-hud" className="absolute left-4 right-4 top-[5.7rem] z-10 flex flex-col gap-3 md:left-6 md:right-6 md:top-[6.6rem] lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-[720px] rounded-3xl border border-sky-100/18 bg-black/42 p-3 shadow-2xl backdrop-blur-md md:p-4">
          ...
          {rivalResponseDecision ? <p data-qa="world-rival-response-banner" data-rival-response-decision={rivalResponseDecision.id} className="mt-2 rounded-2xl border border-red-200/45 bg-red-300/14 px-3 py-2 text-xs font-black uppercase tracking-[0.16em] text-red-100">⚔ Rival response · {rivalResponseDecision.label}</p> : rivalResponse ? <p data-qa="world-rival-response-ready" data-rival-response={rivalResponse.id} className="mt-2 rounded-2xl border border-red-200/40 bg-red-300/10 px-3 py-2 text-xs font-black uppercase tracking-[0.16em] text-red-100">⚔ Rival response pending · {rivalResponse.rival}</p> : null}
      ...
      <div className="absolute bottom-[5.2rem] left-3 right-3 top-[14.8rem] flex flex-col gap-3 overflow-y-auto pb-6 md:bottom-[6.2rem] md:left-6 md:right-6 md:top-[13.8rem] lg:grid lg:grid-cols-[minmax(0,1fr)_400px] lg:overflow-hidden lg:pb-0">
        <div className="shrink-0 rounded-[2rem] border border-sky-100/16 bg-black/28 p-3 shadow-[0_30px_90px_rgba(0,0,0,.48)] backdrop-blur-sm md:p-4 lg:min-h-0">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {courtDecision ? <Legend label="Court ruling" tone="border-emerald-200/80 bg-emerald-300/18 text-emerald-50" /> : null}
            {rivalResponseDecision ? <Legend label="Rival response" tone="border-red-200/80 bg-red-300/18 text-red-50" /> : null}
        ...
        <aside data-qa="world-sector-inspect" data-sector-control={selectedControl} className="shrink-0 rounded-[2rem] border border-amber-100/16 bg-black/48 p-3 shadow-2xl backdrop-blur-md md:p-4 lg:min-h-0 lg:overflow-auto">
          ...
          {courtDecision ? <div data-qa="world-court-ruling" data-court-case={courtCase?.id ?? "none"} data-court-case-decision={courtDecision.id} className="mt-3 rounded-2xl border border-emerald-200/35 bg-emerald-300/12 p-3"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-100/65">Court ruling on the map</p><p className="mt-1 text-sm font-black text-amber-50">{courtDecision.label}</p><p className="mt-1 text-xs leading-relaxed text-amber-50/62">{courtDecision.worldEffect}</p></div> : null}
          {rivalResponseDecision ? <div data-qa="world-rival-response" data-rival-response={rivalResponse?.id ?? "none"} data-rival-response-decision={rivalResponseDecision.id} className="mt-3 rounded-2xl border border-red-200/40 bg-red-300/14 p-3"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-red-100/70">Rival response on the map</p><p className="mt-1 text-sm font-black text-amber-50">{rivalResponse?.rival}: {rivalResponseDecision.label}</p><p className="mt-1 text-xs leading-relaxed text-amber-50/62">{rivalResponseDecision.worldEffect}</p></div> : rivalResponse ? <div data-qa="world-rival-response-pending" data-rival-response={rivalResponse.id} className="mt-3 rounded-2xl border border-red-200/35 bg-red-300/10 p-3"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-red-100/65">Rival response pending</p><p className="mt-1 text-sm font-black text-amber-50">{rivalResponse.rival}</p><p className="mt-1 text-xs leading-relaxed text-amber-50/62">{rivalResponse.prompt}</p></div> : null}
